...
function populateTable(data) {
    ...
    data.forEach((student) => {
        ...
        row.innerHTML = `
            <td>${student.id}</td>
            <td><img src="${student.img_src}" alt="${student.first_name} ${student.last_name}" width="50" height="50"> ${student.first_name} ${student.last_name}</td>
...
if (searchButton) {
    searchButton.addEventListener('click', filterAndUpdateTable);
}

// Function to sort the student data by full name (ascending A->Z)
function sortByNameAsc() {
    studentData.sort((a, b) => (a.first_name + ' ' + a.last_name).localeCompare(b.first_name + ' ' + b.last_name));
    populateTable(studentData);
}

// Function to sort the student data by full name (descending Z->A)
function sortByNameDesc() {
    studentData.sort((a, b) => (b.first_name + ' ' + b.last_name).localeCompare(a.first_name + ' ' + a.last_name));
    populateTable(studentData);
}

// Function to sort the student data by marks (ascending)
...
// Function to sort the student data by class (ascending)
function sortByClassAsc() {
    studentData.sort((a, b) => a.class - b.class);
    populateTable(studentData);
}

// Function to show male and female students in separate tables


// Add event listeners for the sorting and filtering buttons
document.getElementById('sortByNameAsc').addEventListener('click', sortByNameAsc);
document.getElementById('sortByNameDesc').addEventListener('click', sortByNameDesc);
document.getElementById('sortByMarksAsc').addEventListener('click', sortByMarksAsc);
document.getElementById('showPassingStudents').addEventListener('click', showPassingStudents);
document.getElementById('sortByClassAsc').addEventListener('click', sortByClassAsc);




// Function to create a table and populate it with data
function createAndPopulateTable(data, id) {

    const table = document.createElement('table');
    const tbody = document.createElement('tbody');

    table.appendChild(document.getElementById('tableBody').previousElementSibling.cloneNode(true));
...
// Function to show male and female students in separate tables
function showMaleFemaleStudents() {
    ...
    // Create and populate tables for male and female students
    createAndPopulateTable(femaleStudents, 'tableContainer');
    createAndPopulateTable(maleStudents, 'tableContainer');
}

// Add event listener for sorting and filtering buttons
document.getElementById('showMaleFemaleStudents').addEventListener('click', showMaleFemaleStudents);
